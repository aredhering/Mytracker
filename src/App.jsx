...
function FloatingPlayer() {
  ...
  const currentAudio = () => getAudios()[trackIdx];
  ...
  const goToTrack = (idx) => {
    ...
    const prevAudio = audios[trackIdx];
    const nextAudio = audios[idx];
    prevAudio.pause();
  ...
  const prev = () => goToTrack((trackIdx - 1 + TRACKS.length) % TRACKS.length);
  const next = () => goToTrack((trackIdx + 1) % TRACKS.length);
  ...
  useRef(() => {
    const interval = setInterval(() => {
      if (audiosRef.current) onTimeUpdate();
    }, 500);

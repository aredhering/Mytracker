...
// Royalty-free tracks — all CC0/public domain from Pixabay
const TRACKS = [
  {
  ...
  },
  {
    title: "Calm Morning",
    artist: "FASSounds",
    url: "https://cdn.pixabay.com/download/audio/2022/03/10/audio_270f49c6e5.mp3",
    color: "#38bdf8",
  },
  {
    title: "Lofi Chill",
    artist: "Lesfm",
    url: "https://cdn.pixabay.com/download/audio/2022/01/18/audio_d0c6ff1fbe.mp3",
    color: "#f472b6",
  },
  {
    title: "Soft Piano",
    artist: "Relaxing Music",
    url: "https://cdn.pixabay.com/download/audio/2021/11/01/audio_cb31e41351.mp3",
    color: "#34d399",
  },
  {
    title: "Night Ambient",
  ...
  },
  {
    title: "Dreamy Lofi",
    artist: "Lesfm",
    url: "https://cdn.pixabay.com/download/audio/2022/10/25/audio_946b4a3e4e.mp3",
    color: "#fb923c",
  },
];
...
function FloatingPlayer() {
  ...
  const onLoadedMetadata = () => {
    const a = audioRef.current;
    if (a) { setDuration(a.duration); setLoading(false); }
  };
  const onEnded = () => next();
    ...
    <>
      ...
      <div style={{
        ...
        <div style={{height:66, display:"flex", alignItems:"center", gap:12, padding:"0 16px"}}>
          ...
          {/* Album art placeholder with color */}
          <div
            onClick={()=>setExpanded(e=>!e)}
            style={{width:40,height:40,borderRadius:10,background:`linear-gradient(135deg,${track.color}44,${track.color}22)`,border:`1px solid ${track.color}55`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:18,flexShrink:0,cursor:"pointer"}}
          >
            {playing ? "🎵" : "🎶"}
          </div>
          ...
          <button onClick={togglePlay} style={{
            ...
            boxShadow: playing ? `0 0 14px ${track.color}88` : "none",
        ...
        <div style={{padding:"0 20px 16px"}}>
          ...
          <div style={{display:"flex",gap:3,alignItems:"flex-end",justifyContent:"center",height:32,marginBottom:14}}>
            {Array.from({length:32}).map((_,i)=>{
              ...
              return (
                <div key={i} style={{
                  width:4, height:`${h}px`,
                  background: playing ? `${track.color}${Math.round(60+i*3).toString(16).padStart(2,"0")}` : "#1e293b",
                  borderRadius:2,
                  transition:"background .3s",
                  animation: playing ? `eqBounce ${0.4+(i%5)*0.12}s ${i*0.03}s ease-in-out infinite alternate` : "none",
                }}/>
              );
            })}
          </div>
          ...
          <div style={{display:"flex",gap:6,overflowX:"auto",paddingBottom:2}}>
            {TRACKS.map((t,i)=>(
              <button key={i} onClick={()=>{setTrackIdx(i); setPlaying(true); setTimeout(()=>{ audioRef.current?.play().catch(()=>{}); },50);}} style={{
                flexShrink:0,
                padding:"5px 12px",
                borderRadius:20,
                border:`1px solid ${i===trackIdx ? t.color+"88":"rgba(255,255,255,.08)"}`,
                background: i===trackIdx ? `${t.color}22` : "rgba(255,255,255,.04)",
                color: i===trackIdx ? t.color : "#64748b",
                fontSize:11, fontWeight: i===trackIdx?700:400,
...
export default function DailyTracker() {
  ...
  const [log, setLog] = useState(() => { try { return JSON.parse(localStorage.getItem("dl_log")||"{}"); } catch { return {}; } });
  const [activeTab, setActiveTab] = useState("today");

  const saveLog = (nl) => { setLog(nl); try { localStorage.setItem("dl_log",JSON.stringify(nl)); } catch {} };
  ...
  return (
    <div style={{minHeight:"100vh",background:"#080b14",fontFamily:"'DM Sans','Segoe UI',sans-serif",color:"#e2e8f0",position:"relative",overflow:"hidden"}}>
      ...
      {/* Floating player — always mounted */}
      <FloatingPlayer />

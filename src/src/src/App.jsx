...
const getStorage = () => {
  try { return JSON.parse(localStorage.getItem("dl_log")||"{}"); } catch { return {}; }
};
const setStorage = (data) => {
  try { localStorage.setItem("dl_log", JSON.stringify(data)); } catch {}
};
...
function FloatingPlayer() {
  ...
  const togglePlay = () => {
    ...
    else { setLoading(true); audio.play().then(() => { setPlaying(true); setLoading(false); }).catch(() => { setPlaying(false); setLoading(false); }); }
  };

  const prev = () => playTrack((trackIdx - 1 + TRACKS.length) % TRACKS.length);
  const next = () => playTrack((trackIdx + 1) % TRACKS.length);

  const onTimeUpdate = () => { const a = audioRef.current; if (a && a.duration) setProgress(a.currentTime / a.duration); };
  const onLoadedMetadata = () => { const a = audioRef.current; if (a) { setDuration(a.duration); setLoading(false); } };
  const onEnded = () => next();
  const onCanPlay = () => setLoading(false);
  const seekTo = (e) => { const rect = e.currentTarget.getBoundingClientRect(); const pct = (e.clientX - rect.left) / rect.width; const audio = audioRef.current; if (audio && audio.duration) { audio.currentTime = pct * audio.duration; setProgress(pct); } };
  const fmtTime = (s) => `${Math.floor(s/60)}:${String(Math.floor(s%60)).padStart(2,"0")}`;
  const playerH = expanded ? 210 : 68;

  return (
    <>
      <audio ref={audioRef} onTimeUpdate={onTimeUpdate} onLoadedMetadata={onLoadedMetadata} onEnded={onEnded} onCanPlay={onCanPlay} preload="metadata" />
      <div style={{position:"fixed",bottom:0,left:0,right:0,zIndex:100,background:"rgba(6,9,18,0.97)",backdropFilter:"blur(20px)",borderTop:`1px solid ${track.color}33`,height:playerH,overflow:"hidden",transition:"height .35s cubic-bezier(.4,0,.2,1)",boxShadow:"0 -8px 40px rgba(0,0,0,.7)"}}>
        ...
        <div style={{height:66,display:"flex",alignItems:"center",gap:12,padding:"0 16px"}}>
          ...
          <div style={{flex:1,minWidth:0,cursor:"pointer"}} onClick={()=>setExpanded(e=>!e)}>
            ...
            <div style={{fontSize:10,color:"#475569",marginTop:1}}>{track.artist} · royalty-free</div>
          </div>
          <button onClick={prev} style={{background:"none",border:"none",color:"#64748b",fontSize:16,cursor:"pointer",padding:"4px",lineHeight:1}}>⏮</button>
          <button onClick={togglePlay} style={{width:38,height:38,borderRadius:"50%",border:"none",cursor:"pointer",background:`linear-gradient(135deg,${track.color},${track.color}99)`,color:"#fff",fontSize:14,display:"flex",alignItems:"center",justifyContent:"center",boxShadow:playing?`0 0 14px ${track.color}88`:"none",transition:"box-shadow .2s",flexShrink:0}}>
            {loading ? "⟳" : playing ? "⏸" : "▶"}
          </button>
          <button onClick={next} style={{background:"none",border:"none",color:"#64748b",fontSize:16,cursor:"pointer",padding:"4px",lineHeight:1}}>⏭</button>
        ...
        <div style={{padding:"0 20px 16px"}}>
          <div style={{display:"flex",gap:3,alignItems:"flex-end",justifyContent:"center",height:32,marginBottom:14}}>
            {Array.from({length:32}).map((_,i)=>{
              const h = 4 + Math.abs(Math.sin(i*0.8 + trackIdx)*20);
              return <div key={i} style={{width:4,height:`${h}px`,background:playing?`${track.color}${Math.round(60+i*3).toString(16).padStart(2,"00")}`:"#1e293b",borderRadius:2,transition:"background .3s",animation:playing?`eqBounce ${0.4+(i%5)*0.12}s ${i*0.03}s ease-in-out infinite alternate`:"none"}}/>;
            })}
          </div>
    ...
    <div style={{minHeight:"100vh",background:"#080b14",fontFamily:"'DM Sans','Segoe UI',sans-serif",color:"#e2e8f0",position:"relative",overflow:"hidden"}}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0;}
        ...
        @keyframes eqBounce{from{transform:scaleY(0.4)}to{transform:scaleY(1.0)}}
